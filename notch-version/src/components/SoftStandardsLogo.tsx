...
interface SoftStandardsLogoProps {
  theme?: 'light' | 'dark';
  className?: string;
  autoPlay?: boolean;
}

export default function SoftStandardsLogo({
  theme = 'light',
  className = '',
  autoPlay = true
}: SoftStandardsLogoProps) {
  ...
  const bgColor = theme === 'light' ? '#F7F5FE' : '#00002D';
  ...
  return (
    ...
    >
      ...
      {/* Background circle/rectangle based on theme */}
      {theme === 'light' ? (
        <circle cx="1000.63" cy="1000.03" r="1000" fill={bgColor} />
      ) : (
        <rect
          x="8.98047"
          y="8.34668"
      ...
      <clipPath id="logoClip">
        <rect width="2000" height="2000" fill="white" transform="translate(0.632812 0.0302734)" />
      </clipPath>
    </svg>
  );
}
